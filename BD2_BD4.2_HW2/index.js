...
const dbFilePath = path.join(__dirname, "bd4.2_tracks_database.sqlite");
...
app.get("/tracks", async (req, res) => {
  try {
    ...
    if (results.tracks.length === 0) {
      res.status(404).json({ message: "Not Get any Tracks" });
    }
...
app.get("/tracks/artist/:artist", async (req, res) => {
  try {
    ...
    if (results.tracks.length === 0) {
      res.status(404).json({ message: "Not Get any tracks of this artist" });
    }
    res.status(200).json(results);
  } catch (err) {
    return res.status(500).json({ error: "Internal Server Error" });
...
app.get("/tracks/genre/:genre", async (req, res) => {
  try {
    ...
    if (results.tracks.length === 0) {
      res.status(404).json({ message: "Not Get any tracks of genre" });
    }
    res.status(200).json(results);
  } catch (error) {
    return res.status(500).json({ error: "Internal Server Error" });
...
app.get("/tracks/release_year/:year", async (req, res) => {
  try {
    ...
    if (results.tracks.length === 0) {
      res
        .status(404)
        .json({ message: "Not Get any tracks release in this year" });
    }
    res.status(200).json(results);
  } catch (error) {
    return res.status(500).json({ error: "Internal Server Error" });
